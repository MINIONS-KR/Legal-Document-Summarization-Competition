...
class ScoreEarlyStopper():
    """Early stopper
    
    Attributes:
        patience (int): score가 늘어나지 않아도 학습할 epoch 수
        verbose (bool): 로그 출력 여부, True 일 때 로그 출력
        patience_counter (int): score가 늘어나지 않을 때 마다 1씩 증가
        best_score (float): 최소 score
        stop (bool): True 일 때 학습 중단
    ...
    def check_early_stopping(self, score: float)-> None:
        if self.best_score == 0:
            ...
            self.save_model = True
            self.patience_counter = 0
            msg = f"Early stopper, Validation Score Increased {self.best_score} -> {score}"
            self.best_score = score
